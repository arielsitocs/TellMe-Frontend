...
export default function SideNavigationLink({ icon, title, linkTo, action }: SideNavigationLinkProps) {
    const titleClass = title === "Cerrar Sesión"
        ? "text-red text-[15px] font-medium"
        : "text-light-gray text-[15px] font-medium"

    return (
        <Link href={`/${linkTo}`} className="flex items-center gap-2 p-3 hover:bg-main-purple transition-all border-b-1 border-borders" onClick={() => action?.()}>
